...
class CachingServer(TCPServer):
    ...
    def touchItem(self, path: str):
        ...
        print("the cachetable is:", self.cacheTable)
        if path in self.cacheTable:
            self.log_info(f"the file of {path} is in cacheTbale!!!")
            if self.cacheTable.expired(path): #target expired, ask mainserver
                response = self.requestMainServer(path)
                if response is None:
                    yield None
                    return
                print(f"update the {path}in cachetable")
                '''
                headers = response.getheaders()
                rawfile = response.read()
                self.cacheTable.setHeaders(path, headers)
                self.cacheTable.appendBody(path, rawfile)
                return headers, rawfile
                '''
                the_headers = response.getheaders()
                self.cacheTable.setHeaders(path, the_headers)
                yield the_headers
                buffer = bytearray(BUFFER_SIZE)
                while True:
                    size = response.readinto(buffer)
                    if size == 0:
                        break
                    self.cacheTable.appendBody(path,  buffer[:size])
                    yield buffer[:size]
            else:
                yield self.cacheTable.getHeaders(path)
                yield self.cacheTable.getBody(path)
        else:#ask mainserver
            print(f"the file of {path} isn't in cacheTbale!!!")
            '''
            response = self.requestMainServer(path)
            if response is None:
                return None, None
            print(f"add the {path} in cachetable")
            headers = response.getheaders()
            rawfile = response.read()
            self.cacheTable.setHeaders(path, headers)
            self.cacheTable.appendBody(path, rawfile)
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    def sendHeaders(self, headers):
        ...
        headers = self.server._filterHeaders(headers)
        print("do_GET and get headers:", headers)
        self.send_response(HTTPStatus.OK)
        for header, value in headers: #headers is the list of (head, value)
    ...
    def do_GET(self):
        ...
        first = next(item)
        if first is None: #a empty
            print("can't find in server")
            self.send_error(HTTPStatus.NOT_FOUND, "'File not found'")
            return 
        else: 
            self.sendHeaders(first)#first item is headers
            while True: #else is body
                try:
                    body = next(item)
                    self.sendBody(body)
    ...
    def do_HEAD(self):
        ...
        # Similar to do_GET()
        '''
        headers, _ = self.server.touchItem(self.path)
        if headers != None:
            self.sendHeaders(headers)
        else:
            self.send_error(HTTPStatus.NOT_FOUND, "File not found")
        ...
        first = next(item)
        if first is None:
            print("cna't find in server")
            self.send_error(HTTPStatus.NOT_FOUND, "'File not found'")
            return 
        else:
            self.sendHeaders(first)
